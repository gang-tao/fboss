...
constexpr auto kNewJsonBspPlatformMappingStr = R"(
{
  "pimMapping": {
    "1": {
      "pimID": 1,
...
const char* getBspPlatformMappingStr() {
  const char* rtmOldMapping = std::getenv("RTM_OLDMAPING");
  if (rtmOldMapping && std::string(rtmOldMapping) == "1") {
    XLOG(DBG2) << "Gangtao retimer oldmaping";
    return kJsonBspPlatformMappingStr;
  }
  XLOG(DBG2) << "Gangtao retimer newmaping";
  return kNewJsonBspPlatformMappingStr;
}
...
Ladakh800bclsBspPlatformMapping::Ladakh800bclsBspPlatformMapping()
    : BspPlatformMapping(
          buildLadakh800bclsPlatformMapping(getBspPlatformMappingStr())) {}
